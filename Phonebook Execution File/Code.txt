...
int read_contact(FILE *fp)
{
    ...
    {
        i=lines;
        fscanf(fp,"%s%s%s%s%d%d%d%d",c[i].firstname,c[i].lastname,c[i].address,c[i].email,&c[i].phone,&c[i].bd.day,&c[i].bd.month,&c[i].bd.year);
        lines++;
        if(feof(fp))
            break;
    }
...
int sort_cont(FILE *fp,int mode)
{
    int length=lines;
    int pass;
    contact temp;
    ...
    else if(mode==2)
    {
        for(pass=1; pass<length; pass++)
        {
            for(i=0; i<length-pass; i++)
            {
                if(strcmp(c[i].lastname,c[i+1].lastname)==1)
                {
                    temp=c[i];
                    c[i]=c[i+1];
                    c[i+1]=temp;
                }
                else if(!(strcmp(c[i].lastname,c[i+1].lastname))&&strcmp(c[i].firstname,c[i+1].firstname)==1)
                {
                    temp=c[i];
                    c[i]=c[i+1];
                    c[i+1]=temp;
...
void search_contacts(FILE *fp,char *target)
{
    int i;
    contact temp[lines];
    int counter=0;
...
void delete_contact(FILE *fp,char *target)
{
    int i,pos;
    int check = 0;
    for(i=0; i<lines; i++)
...
void modify(FILE *fp,char *target)
{
    int i,ch,p[lines];
    contact temp[lines];
    int counter=0;
...
int main()
{
    ...
    while(1)
    {
        ...
        case 6:
            ...
            scanf("%d",&ch2);
            printf ("\n");
            print_contacts(sort_cont(fp,ch2));
            break;
        case 7:
            save(fp,lines);
            break;
